...
const outputDataDir = path.join(projectRoot, "src", "data", "game");
...
const wikiFiles = (await walk(path.join(projectRoot, "src", "data", "wiki"))).filter(
...
function collectNames(value) {
  ...
  if (typeof value.name === "string") wikiNames.add(value.name.toLowerCase());
  for (const child of Object.values(value)) collectNames(child);
}

for (const filePath of wikiFiles) {
  collectNames(JSON.parse(await fs.readFile(filePath, "utf8")));
}
